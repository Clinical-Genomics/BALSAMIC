...
def process_vcf(input_vcf: str, output_vcf: str):
    ...
    if sample_name != 'TUMOR':
        LOG.warning(f"Error: The first sample is named '{sample_name}', but 'TUMOR' is expected.")
        sys.exit(1)

    # Add AF and DP fields to the header if not already present
    if 'AF' not in reader.header.info_ids():
        reader.header.add_info_line(
            vcfpy.OrderedDict([('ID', 'AF'), ('Number', 'A'), ('Type', 'Float'), ('Description', 'Allele Frequency')])
        )

    if 'DP' not in reader.header.info_ids():
        reader.header.add_info_line(
            vcfpy.OrderedDict([('ID', 'DP'), ('Number', '1'), ('Type', 'Integer'), ('Description', 'Total Depth')])
        )
    ...
    with vcfpy.Writer.from_path(output_vcf, reader.header) as writer:
        ...
        for record in reader:
            ...
            tumor_ad: Optional[List[int]] = tumor_call.data.get('AD', None)  # AD is a list [ref_count, alt_count]
            if tumor_ad is None:
                LOG.warning(f"Warning: AD field is missing for record at position {record.POS} on {record.CHROM}")
            else:
            ...
            if tumor_af is None:
                LOG.warning(f"Warning: AF field is missing for record at position {record.POS} on {record.CHROM}")
