...
@click.option(
    "--snv-vcf", default="vcfmerge", help="variant caller to load as vcf_cancer"
)
...
@click.option(
    "--sv-vcf", default="manta", help="variant caller to load as vcf_cancer_sv"
)
...
def scout(context, sample_config, snv_vcf, sv_vcf, customer_id, tumor, normal):
    ...
    LOG.info("Scout config template is successfully created: %s", scout_config_dst)
