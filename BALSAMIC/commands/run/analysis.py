...
@click.option('-a', '--analysis-type', required=False, type=click.Choice(['qc', 'paired',
              'single', 'paired_umi']), help='Type of analysis to run from input config file.\
              By default it will read from config file, but it will override config file \
              if it is set here.')
@click.option('-S', '--snake-file', type=click.Path(), show_default=True,
...
@click.option('-s', '--sample-config', required=True, type=click.Path(),
              help='Sample json config file.')
@click.option('--run-mode', show_default=True, default='slurm', type=click.Choice(["local",
              "slurm"]), help='Run mode to use. By default SLURM will be used to run the analysis.\
              But local runner also available for local computing')
@click.option('-c', '--cluster-config', show_default=True, default=get_config('cluster'),
              type=click.Path(), help='SLURM config json file.')
@click.option('-l', '--log-file', type=click.Path(), help='Log file output for BALSAMIC.\
...
@click.option('--slurm-mail-type', type=click.Choice(['NONE', 'BEGIN', 'END',
              'FAIL', 'REQUEUE', 'ALL', 'TIME_LIMIT']), help='SLURM mail type to send out email.\
