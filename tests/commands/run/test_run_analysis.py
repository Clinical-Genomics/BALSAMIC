...
def test_run_analysis_tumor_only_dog_dry_run(
    invoke_cli, tumor_only_dog_config
):
    # GIVEN a tumor-only config file
    # WHEN running analysis
    result = invoke_cli(
        ["run", "analysis", "-s", tumor_only_dog_config, "--mail-type", "FAIL"]
    )

    # THEN it should run without any error
    assert result.exit_code == 0

def test_run_analysis_tumor_normal_dog_dry_run(
    invoke_cli, tumor_normal_dog_config
):
    # GIVEN a tumor-only config file
    # WHEN running analysis
    result = invoke_cli(
        ["run", "analysis", "-s", tumor_normal_dog_config, "--mail-type", "FAIL"]
    )
