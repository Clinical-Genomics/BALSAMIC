...
def test_deliver_tumor_only_panel(invoke_cli, tumor_only_config, helpers, caplog):
    ...
    # minimal files your Snakefile expects to exist
    multiqc_data_dir = Path(helpers.result_dir, "qc", "multiqc_data")
    multiqc_data_dir.mkdir(parents=True, exist_ok=True)
    (multiqc_data_dir / "multiqc_data.json").touch()
    ...
    # Make this robust: check all captured log records instead of a raw substring
    assert any(
        "following" in (rec.getMessage() or "").lower() for rec in caplog.records
    ), f"'following' not found in logs. Got logs:\n{caplog.text}"
